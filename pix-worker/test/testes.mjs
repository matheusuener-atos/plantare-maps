...
const env={ MP_ACCESS_TOKEN:'TEST-123', MP_WEBHOOK_SECRET:'segredo', ORIGENS:'https://plantare.matheusuener.com.br', CUPONS:'{"CAMPO10":0.10}', EXPIRA:'PT30M' };
...
globalThis.fetch=async(url,op)=>{ ultimo={url,op};
  ...
  if(String(url).includes('/v1/orders/ORD01ABC123')) return new Response(JSON.stringify({id:'ORD01ABC123', status:'processed', status_detail:'accredited', external_reference:'plantare|4K4D1L|ABP|100.00', total_amount:'900.00'}),{status:200});
...
await t('/cobranca cria order Pix com preço do servidor', async()=>{ const r=await worker.fetch(req('/cobranca',{plano:'4K4D1L',polys,camadas:['linhas_ab','bordadura','percurso'],email:'a@b.com',valor:1}),env); const j=await r.json();
  ...
  assert.ok(ultimo.op.headers['x-idempotency-key']); assert.equal(ultimo.op.headers.authorization,'Bearer TEST-123'); assert.match(b.external_reference,/^plantare\|4K4D1L\|ABP\|/); });
